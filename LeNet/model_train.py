...
def train_val_data_process():
    ...
    train_dataloader = Data.DataLoader(dataset=train_data, batch_size=16, shuffle=True, num_workers=2)
    val_dataloader = Data.DataLoader(dataset=val_data, batch_size=16, shuffle=True, num_workers=2)
...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs=5):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (b_x, b_y) in enumerate(train_dataloader):
            ...
            # 反向传播计算
            loss.backward()
            # 更新参数 根据网络反向传播的梯度信息来更新网络的参数，以起到降低loss函数计算值的作用
            optimizer.step()
            # 对损失函数进行累加
        ...
        print("训练和验证用时{:.0f}m {:.0f}s".format(time_use // 60, time_use % 60))
    ...
    # 将准确率和损失值从GPU转移到CPU
    train_acc_all = [acc.cpu().item() for acc in train_acc_all]
    val_acc_all = [acc.cpu().item() for acc in val_acc_all]
...
if __name__ == "__main__":
    ...
    # 利用现有的模型进行模型的训练
    train_process = train_model_process(LeNet, train_data, val_data, num_epochs=500)
